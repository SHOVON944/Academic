...
class Restaurant {
    ...
    public void placeOrder(String foodItem, int quantity) throws OutOfStockException, InvalidQuantityException {
        if (quantity <= 0) throw new InvalidQuantityException("Quantity must be > 0.");

        if (foodItem.equalsIgnoreCase("Pizza")) {
            if (quantity > pizzaStock) throw new OutOfStockException("Pizza is out of stock.");
            pizzaStock -= quantity;
        } else if (foodItem.equalsIgnoreCase("Burger")) {
            if (quantity > burgerStock) throw new OutOfStockException("Burger is out of stock.");
...
public class P_29 {
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (resName.equalsIgnoreCase("exit")) break;
