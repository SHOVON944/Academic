...
void POLISH(string Q){
    ...
    //TODO  step 2: scan Q [left -> right]
    for(int i=0; i<Q.length(); i++){
        char ch = Q[i];

        if(isALphabateDigit(ch)){    //TODO Step 3: If operand -> add to P
            P = P + ch;
        } else if(ch=='('){     //TODO Step 4: If left parenthesis, push into STACK
            STACK.push(ch);
        } else if(ch=='*'   ||   ch=='/'   ||   ch=='^'   ||   ch=='+'   ||   ch=='-'){ //TODO Step 5: If operator encountered
            while(precedence(STACK.top()) >= precedence(ch)){   //TODO (a) Pop operators of higher or equal precedence
                P = P + STACK.top();
                STACK.pop();
            }
            STACK.push(ch);    //TODO (b) Push operator into STACK
        } else if(ch==')'){  //TODO Step 6: If right parenthesis
            while(STACK.top()!='('){  //TODO (a) Pop until left parenthesis
                P = P + STACK.top();
                STACK.pop();
            }
            STACK.pop();    //TODO // (b) Remove left parenthesis
        }
    }
